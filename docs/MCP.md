...
Codebase Atlas implements the stable MCP protocol revision `2025-11-25` over stdio. Messages are newline-delimited UTF-8 JSON-RPC. The server exposes read-only `definition`, `references`, `callers`, `callees`, `related_tests`, and `impact` tools and returns both serialized text content and structured content.
