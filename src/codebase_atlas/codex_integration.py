...
PROJECT_RULE = """At task start, call project_status once. Stop if its repository is not the open project; tell the user if automatic refresh failed or a software update is available. The on-query policy automatically coalesces one safe refresh after creating, modifying, renaming, or deleting a batch of source files; inspect the returned auto_update status and never ask the user to remember a refresh command. Atlas read/query calls from multiple agents may run concurrently. Give explicit enable, stop, update, remove, migration, index, or refresh work one owning agent; other agents wait for it. Treat provider_busy or provider_startup_timeout as transient backpressure: retry within the remaining task budget after the owner finishes, and do not archive or recreate a task merely to recover. Before changing code, use ordinary source search to discover an exact candidate. Then call Codebase Atlas analyze_change with symbol plus path/owner. Treat unresolved or needs_disambiguation as a stop; never guess. Preserve generation, stale, warning, partial, truncation, continuation, and auto_update fields. Read the returned source regions before editing and run only evidence-backed tests. Fall back to direct source inspection whenever Atlas reports partial, error, or unsupported evidence."""
