...
PROJECT_RULE = """At task start, call project_status once. Stop if its repository is not the open project; tell the user if automatic refresh failed or a software update is available. Before changing code, use ordinary source search to discover an exact candidate. Then call Codebase Atlas analyze_change with symbol plus path/owner. Treat unresolved or needs_disambiguation as a stop; never guess. Preserve stale, warning, partial, truncation, and continuation fields. Read the returned source regions before editing and run only evidence-backed tests. Fall back to direct source inspection whenever Atlas reports partial, error, or unsupported evidence."""
